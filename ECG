...
print("\n" + "="*60)
print("  STEP 1 — ECG Waveform Anatomy (labelled)")
print("="*60)
plot_anatomy()

print("\n" + "="*60)
print("  STEP 2 — Full Atlas (all 15 conditions)")
print("="*60)
plot_all_overview()

print("\n" + "="*60)
print("  STEP 3 — Category Comparisons")
print("="*60)
...
print("\n" + "="*60)
print("  STEP 4 — Individual deep-dive: Critical conditions")
print("="*60)
CRITICAL = [w for w in WAVEFORMS if w["severity"] in ("CRITICAL","FATAL")]
for w in CRITICAL:
    print(f"\n  → Deep-dive: {w['name']}")
    plot_single(w)

print("\n✅ All plots complete! Check sidebar for saved .png files.")


# ─────────────────────────────────────────────────────────────
# CELL 9 — Interactive Selector (Colab widget)
# ─────────────────────────────────────────────────────────────

from ipywidgets import interact, widgets as wg
from IPython.display import clear_output

def interactive_ecg():
    names = [w["name"] for w in WAVEFORMS]
    wf_map = {w["name"]: w for w in WAVEFORMS}

    @interact(
        condition=wg.Dropdown(options=names, description="Condition:",
                               style={"description_width":"initial"},
                               layout=wg.Layout(width="380px")),
        window=wg.FloatSlider(value=5, min=2, max=8, step=0.5,
                               description="Window (s):",
                               style={"description_width":"initial"},
                               layout=wg.Layout(width="380px")),
        show_deep_dive=wg.Checkbox(value=True, description="Show deep-dive"),
    )
    def _update(condition, window, show_deep_dive):
        clear_output(wait=True)
        w = wf_map[condition]
        if show_deep_dive:
            plot_single(w, window=window)
        else:
            fig, ax = plt.subplots(figsize=(14,4), facecolor="#071207")
            ax.set_facecolor("#071207")
            sig = w["fn"]()
            t   = np.linspace(0, DURATION, len(sig))
            msk = t <= window
            ax.plot(t[msk], sig[msk], color=w["cat_color"], lw=1.2)
            ax.axhline(0, color="#1a5c1a", lw=0.8)
            ax.set_title(condition, color="white", fontfamily="monospace")
            ax.set_facecolor("#071207")
            for sp in ax.spines.values(): sp.set_edgecolor("#222")
